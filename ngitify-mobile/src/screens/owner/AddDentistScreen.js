...
export default function AddDentistScreen({ navigation }) {
    ...
    const maxDate21 = new Date();
    maxDate21.setFullYear(maxDate21.getFullYear() - 21);
    const [dateObj, setDateObj] = useState(maxDate21); 
    ...
    const specializations = ["General Dentistry", "Orthodontics", "Pediatric Dentistry", "Endodontics", "Oral Surgery", "Prosthodontics"];
    ...
    const isFormValid = 
        ...
        formData.licenseNumber.length === 7 && formData.specialization !== '' &&
    ...
    const handleLicenseChange = (value) => {
        const sanitized = value.replace(/[^0-9]/g, '');
        handleChange('licenseNumber', sanitized);
        if (sanitized.length > 0 && sanitized.length < 7) setLicenseError('PRC License must be exactly 7 digits');
    ...
    const openSpecDropdown = () => setDropdown({ visible: true, title: 'Specialization', items: specializations.map(s => ({label: s, value: s})), labelKey: 'label', valueKey: 'value', onSelect: (val) => handleChange('specialization', val) });
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Add New Dentist</Text>
            </View>
            ...
            <ScrollView contentContainerStyle={styles.formContainer} showsVerticalScrollIndicator={false}>
                <View style={styles.section}>
                    ...
                    <View style={styles.row}>
                        <View style={{flex: 1, marginRight: 10}}>
                            <Text style={styles.label}>Birthdate (21+) *</Text>
                ...
                <View style={styles.section}>
                    ...
                    <TextInput style={[styles.inputBox, {marginBottom: 5}]} placeholder="e.g. 1234567" keyboardType="number-pad" maxLength={7} value={formData.licenseNumber} onChangeText={handleLicenseChange} />
                    {licenseError !== '' && <Text style={styles.errorText}>{licenseError}</Text>}
                    <Text style={[styles.label, {marginTop: 10}]}>Specialization *</Text>
                    <TouchableOpacity style={styles.inputBox} onPress={openSpecDropdown} activeOpacity={0.7}>
                        <Text style={{color: formData.specialization ? '#333' : '#aaa', fontSize: 14}}>{formData.specialization || 'Select Specialization'}</Text>
                        <Text style={{color: '#888', fontSize: 12}}>▼</Text>
                    </TouchableOpacity>
                </View>
                ...
                <TouchableOpacity style={[styles.registerBtn, !isFormValid && styles.registerBtnDisabled]} onPress={() => setModalVisible(true)} disabled={!isFormValid}>
                    <Text style={[styles.registerBtnText, !isFormValid && {color: '#999'}]}>{isFormValid ? "REGISTER DENTIST" : "FILL VALID & REQUIRED FIELDS"}</Text>
                </TouchableOpacity>
                <View style={{height: 30}}></View>
            </ScrollView>

            {showDatePicker && Platform.OS === 'android' && (
                <DateTimePicker value={dateObj} mode="date" display="default" maximumDate={maxDate21} onChange={handleDateChange} />
            )}
                ...
                <View style={styles.bottomSheetOverlay}>
                    <View style={styles.bottomSheetContainer}>
                        ...
                        <View style={{backgroundColor: 'white', alignItems: 'center', paddingBottom: 20}}>
                            <DateTimePicker value={dateObj} mode="date" display="inline" maximumDate={maxDate21} onChange={handleDateChange} themeVariant="light"/>
                        </View>
                    </View>
                </View>
            </Modal>
                ...
                <View style={styles.bottomSheetOverlay}>
                    <View style={styles.bottomSheetContainer}>
                        ...
                        <FlatList data={dropdown.items} keyExtractor={(item, index) => index.toString()} 
                            renderItem={({ item }) => (
                                <TouchableOpacity style={styles.dropdownItem} onPress={() => { dropdown.onSelect(item.value || item[dropdown.valueKey]); setDropdown({...dropdown, visible: false}); }}>
                                    <Text style={styles.dropdownItemText}>{item.label || item[dropdown.labelKey]}</Text>
                                </TouchableOpacity>
                            )}
                        />
                    </View>
            ...
            <CustomModal visible={modalVisible} title="Success!" message={`Dr. ${formData.firstName} ${formData.lastName} has been registered.`} type="success" onClose={() => { setModalVisible(false); navigation.goBack(); }} />
