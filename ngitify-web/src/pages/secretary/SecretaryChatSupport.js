...
import styles from '../../styles/admin/ChatSupport.module.css'; // reuse admin CSS
...
export default function SecretaryChatSupport() {
    ...
    const fetchTickets = useCallback(async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Backend scopes to the secretary's branch via JWT
    ...
    const sendReply = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setSelected(await res.json());
                setReply('');
                fetchTickets(); // refresh list to reflect status open → in-progress
            } else {
    ...
    const assignToMe = () => {
        ...
        updateTicket({ assignedTo: userId, assignedToName: userName });
    };

    const resolveTicket  = () => updateTicket({ status: 'resolved' });
    const reopenTicket   = () => updateTicket({ status: 'open' });
    const closeTicket    = () => updateTicket({ status: 'closed' });
                    ...
                    <>
                        ...
                        <div className={styles.messages}>
                            ...
                            {selected.messages?.map((msg, i) => {
                                // Secretary's messages (and other staff) appear on the right
                                const isStaff = isStaffMessage(msg.senderRole);
                                return (
                                    <div
                                        key={msg._id || i}
                                        className={`${styles.messageBubble} ${isStaff ? styles.bubbleAdmin : styles.bubblePatient}`}
                                    >
                        ...
                        {selected.status !== 'closed' ? (
                            ...
                            <div className={styles.closedNotice}>
                                ...
                                <button className={styles.reopenBtn} onClick={reopenTicket}>
                                    Reopen Ticket
                                </button>
                            </div>
                        )}
