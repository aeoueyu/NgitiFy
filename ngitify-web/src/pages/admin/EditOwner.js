        ...
        <div className={styles.mainOverlay}>
            ...
            <div className={styles.formCard}>
                ...
                <form onSubmit={handleSubmit} noValidate>
                    ...
                    <div className={styles.row}>
                        <div className={styles.formGroup}><label>BIRTHDAY <span style={{ color: 'red' }}>*</span></label><input type="date" className={`${styles.inputField} ${errors.birthday ? styles.errorBorder : ''}`} name="birthday" value={formData.birthday} onChange={handleChange} disabled={isSaving} />{errors.birthday && <span className={styles.errorText}>{errors.birthday}</span>}</div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px', background: formData.isDentist ? '#eff6ff' : '#f8fafc', padding: '14px 16px', borderRadius: '10px', border: `1px solid ${formData.isDentist ? '#bfdbfe' : '#e2e8f0'}` }}>
                        <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px', flexShrink: 0 }}>
                            <input type="checkbox" checked={formData.isDentist} onChange={e => setFormData(prev => ({ ...prev, isDentist: e.target.checked }))} disabled={isSaving} style={{ opacity: 0, width: 0, height: 0 }} />
                    ...
                    {formData.isDentist && (
                        <>
                            ...
                            <div className={styles.row}>
                                <div className={styles.formGroup}><label>LICENSE NO. <span style={{ color: 'red' }}>*</span></label><input className={`${styles.inputField} ${errors.licenseNumber ? styles.errorBorder : ''}`} name="licenseNumber" value={formData.licenseNumber} onChange={handleChange} maxLength={7} disabled={isSaving} />{errors.licenseNumber && <span className={styles.errorText}>{errors.licenseNumber}</span>}</div>
