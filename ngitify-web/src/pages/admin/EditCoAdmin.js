...
export default function EditCoAdmin({ coAdminId, onClose, onSuccess }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const userRes = await authFetch(`/user/${coAdminId}`);
                if (!userRes.ok) {
                    addToast('Failed to load co-administrator data.', 'error');
                    onClose();
                    return;
                }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await authFetch(`/user/${coAdminId}`, {
                method: 'PUT',
            ...
            if (res.ok) {
                ...
                addToast(data.message || 'Failed to update co-administrator.', 'error');
            }
        } catch (error) {
            console.error(error);
            addToast('Cannot connect to server.', 'error');
                    ...
                    <>
                        <div className={styles.headerWrapper}>
                            ...
                            <div className={styles.header}>
                                <h2>Edit <span className={styles.highlight}>Co-Administrator</span></h2>
                                <p>Update the co-administrator's details below.</p>
                            </div>
                        </div>
            ...
            {showSuccessModal && (
                <div className={styles.modalOverlay}>
                    <div className={styles.modalCard}>
                        ...
                        <p className={styles.modalMessage}>The co-administrator's profile has been successfully updated.</p>
