// ngitify-web/src/pages/admin/AddCoAdmin.js
// Same structure as AddBranchManager but POSTs to /add-co-administrator
...
export default function AddCoAdmin({ onClose, onSuccess }) {
    ...
    const handleBlur = (e) => {
        ...
        let newError = '';
        if (name === 'email') { if (!value) newError = 'Required'; else if (!validateEmail(value)) newError = 'Invalid domain'; }
    ...
    const validateForm = () => {
        ...
        ['firstName', 'lastName', 'birthdate', 'email'].forEach(f => { if (!formData[f]) { newErrors[f] = 'Required'; isValid = false; } });
        ...
        const validateAddr = (addr, prefix) => { ['region', 'province', 'city', 'barangay', 'street', 'houseNumber'].forEach(f => { if (!addr[f]) { newErrors[`${prefix}_${f}`] = 'Required'; isValid = false; } }); };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await authFetch('/add-co-administrator', { method: 'POST', body: JSON.stringify(finalData) });
            const data = await response.json();
            if (response.ok) { setShowSuccessModal(true); }
            else { if (response.status === 409) { setErrors(prev => ({ ...prev, [data.field]: data.message })); } else { alert(data.message || 'Failed to add co-admin'); } }
        ...
        <div className={styles.mainOverlay}>
            ...
            <div className={styles.formCard}>
                <div className={styles.headerWrapper}>
                    <button className={styles.backIconButton} onClick={onClose} type="button" disabled={isLoading}><img src={BackIcon} alt="Back" /></button>
                    <div className={styles.header}>
                        <h2>Add New <span className={styles.highlight}>Co-Administrator</span></h2>
                        <p>Enter the co-administrator's personal details below.</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} noValidate>
                    ...
                    {branchOptions.length > 0 && (
                        <>
                            ...
                            <p style={{ color: '#64748b', fontSize: '13px', marginBottom: '15px', marginTop: '-15px' }}>Select the branches this co-admin will oversee.</p>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '20px' }}>
                                {branchOptions.map(branch => (
                                    <label key={branch} style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', padding: '6px 12px', border: `1px solid ${formData.assignedBranches.includes(branch) ? '#01538b' : '#e2e8f0'}`, borderRadius: '6px', backgroundColor: formData.assignedBranches.includes(branch) ? '#e8f4fd' : '#fff', fontSize: '14px' }}>
                    ...
                    <div className={styles.buttonGroup}>
                        ...
                        <button type="submit" className={styles.submitBtn} disabled={isLoading}>{isLoading ? 'ADDING...' : 'ADD CO-ADMINISTRATOR'}</button>
                    </div>
                </form>
            </div>
            ...
            {showSuccessModal && (
                <div className={styles.modalOverlay}>
                    <div className={styles.modalCard}>
                        ...
                        <p className={styles.modalMessage}>New co-administrator has been successfully added. An activation email has been sent.</p>
