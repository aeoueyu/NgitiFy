...
export default function ManageSecretaries() {
    ...
    const totalPages = Math.max(1, Math.ceil(filteredSecretaries.length / rowsPerPage));
    const paginatedSecretaries = filteredSecretaries.slice((page - 1) * rowsPerPage, page * rowsPerPage);
    ...
    return (
        <div className={styles.container}>
            ...
            {!isLoading && filteredSecretaries.length > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', marginTop: '16px', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#475569', fontSize: '14px' }}>
                        <span>Showing {(page - 1) * rowsPerPage + 1} to {Math.min(page * rowsPerPage, filteredSecretaries.length)} of {filteredSecretaries.length}</span>
