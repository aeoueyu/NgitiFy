import React from 'react';
import styles from '../../../styles/dentist/DentalPinStyleOdontogram.module.css';

const UPPER_RIGHT = [18, 17, 16, 15, 14, 13, 12, 11];
const UPPER_LEFT = [21, 22, 23, 24, 25, 26, 27, 28];
const LOWER_RIGHT = [48, 47, 46, 45, 44, 43, 42, 41];
const LOWER_LEFT = [31, 32, 33, 34, 35, 36, 37, 38];

const DISPLAY_CONFIG = {
    1: { scale: 0.65 },
    2: { scale: 0.62 },
    3: { scale: 0.62 },
    4: { scale: 0.62 },
    5: { scale: 0.62 },
    6: { scale: 0.95 },
    7: { scale: 1 },
    8: { scale: 1.2 },
};

const LATERAL_PATHS = {
    1: {
        viewBox: '0 0 46 134',
        crown: 'M39.2326 94.6438C33.1163 85.5409 14.1409 86.0457 6.65385 94.6438C-0.669089 102.084 0.287887 121.08 3.03399 127.814C15.6411 135.545 38.1091 133.675 43.8511 128.437C46.722 121.828 43.8511 103.747 39.2326 94.6438Z',
        roots: ['M18.1375 13.8391C16.1403 42.3702 9.64956 79.4307 6.65381 94.3946C18.9498 99.8859 26.179 100.443 39.8567 95.5168C40.4808 78.9319 40.0564 39.577 33.3659 14.8367C26.953 -8.87723 18.913 2.76033 18.1375 13.8391Z'],
        pulp: 'M24.6596 4.92303C30.269 4.6849 31.2334 104.288 22.1317 95.1425C19.5389 92.7118 23.1687 67.5969 24.3785 42.7689C25.3455 22.9252 23.5958 4.9682 24.6596 4.92303Z',
        gumLine: 'M 0,95 Q 23,100 46,95',
    },
    2: {
        viewBox: '0 0 40 125',
        crown: 'M36.0861 86.4908C33.7138 80.3598 11.2384 78.6075 6.61846 84.8639C0.82482 92.7098 0.250476 113.268 1.62396 116.146C2.99745 119.024 8.866 122.152 18.6053 123.653C28.3446 125.155 35.7115 121.526 37.959 117.647C40.2066 113.768 38.4585 92.6219 36.0861 86.4908Z',
        roots: ['M12.487 53.0815C10.4083 62.2149 9.40701 74.1863 8.61621 83.112C16.2849 89.5132 36.3357 86.7405 36.3357 86.7405C36.3357 86.7405 34.7467 75.1037 35.0871 71.475C37.6081 44.5978 35.3738 25.049 34.213 18.4214C32.6315 12.4571 28.4694 0.328393 22.476 1.0291C14.9842 1.90499 18.3555 19.2973 17.7312 26.8049C17.0027 35.5656 14.1102 45.9493 12.487 53.0815Z'],
        pulp: 'M23.8494 41.5699C24.8167 21.6581 21.6019 4.15698 23.8495 4.15698C28.1612 4.15698 29.9677 35.6889 27.7202 60.2138C26.8687 78.8752 25.6047 94.7422 20.9135 92.2458C17.2318 87.6165 22.6392 66.4831 23.8494 41.5699Z',
        gumLine: 'M 0,85 Q 20,92 40,85',
    },
    3: {
        viewBox: '0 0 47 146',
        crown: 'M44.5622 115.129C42.5478 107.137 40.7902 95.6772 22.907 95.8989C3.61725 96.1381 1 114.898 1 129.988C1 136.106 7.90619 141.245 12.8348 143.598C17.3509 145.755 30.8565 145.156 34.2382 143.598C37.7635 141.975 41.3094 141.201 44.5622 135.108C46.8285 130.862 46.0953 121.211 44.5622 115.129Z',
        roots: ['M9.09261 76.4153C8.17217 80.9327 7.17502 95.9485 6.7915 102.892C6.7915 102.892 16.3143 107.776 22.907 107.886C29.8761 108.003 40.0297 102.892 40.0297 102.892C38.1121 97.8724 34.7883 71.2707 35.4275 67.1299C36.0667 62.989 34.4048 42.9121 33.5099 38.3948C32.6151 33.8775 30.3139 20.5764 29.9304 15.0552C29.5469 9.53405 25.9674 0.875868 22.3879 1.00135C18.8084 1.12683 19.0641 4.01289 18.1692 5.51866C17.2743 7.02443 17.1465 16.561 16.763 20.5764C16.3795 24.5917 16.3795 25.5956 14.5897 29.36C12.8 33.1244 11.7772 43.7903 11.7772 47.0528C11.7772 50.3153 10.2432 70.7687 9.09261 76.4153Z'],
        pulp: 'M19.3901 108.569C20.4638 72.4818 22.0439 6.61914 22.4697 6.61914C26.6797 6.61914 26.8139 49.0405 26.9928 84.7355C27.0356 93.2851 27.8031 101.605 28.0152 108.731C28.0857 111.1 26.141 113.006 23.7516 113.006C21.2913 113.006 19.3176 111.008 19.3901 108.569Z',
        gumLine: 'M 0,103 Q 23,110 47,103',
    },
    4: {
        viewBox: '0 0 40 100',
        crown: 'M33.75 62.5c-3.75,-4 -23.5,-4 -26.5,0 -5.25,7 -6,15.25 -6,19 -0.25,2.75 2.5,8.25 17,10 18,2 20.5,-7.5 20.5,-9.5 0,-3.5 -1,-15.25 -5.25,-19.5z',
        roots: ['M21.5 27.75c2.5,-7.75 4,-13.75 4.5,-17.75 0.25,-1.75 -1.5,-8.5 0.5,-8.5 2.25,0 8.25,8 9,13.5 0.75,5.25 -0.5,22.25 -1.25,29 -0.75,5.5 -0.75,14.25 -0.5,18 -12.5,7.5 -18,7 -26.5,0.5 0,0 2,-3.75 2.5,-12.5 0.25,-5.25 -1.5,-14.25 -1.75,-20.25 -1.25,-16.25 2.75,-27.75 5.5,-28.25 3.5,0.25 3.5,18 8,25.75l0.25 0.5'],
        pulp: 'M19.25 38.5c0.25,1 0,1.75 1.25,2.5 1.75,-2.5 5.75,-9.25 7,-18 1.75,-11 0.75,-14.5 0.75,-15.25 0,-0.5 4,-1 2.5,14 -1.5,15.25 -4.25,21.5 -5,26.75 -0.75,5.25 -1.25,6 -1.75,13.5 -0.75,6.5 -9.25,5.25 -9.25,-0.75 0,-1.5 0,-5.25 0,-7.5 0,-2.75 -3.5,-18 -3.75,-26.25 -0.25,-6.5 0,-20.25 1.25,-21.25 1.5,-1.5 2.75,13.25 3.75,19.25 1,6.25 2.25,8.5 2.75,10.25l0.25 1.75z',
        gumLine: 'M 0,62.5 Q 20,67.5 40,62.5',
    },
    5: {
        viewBox: '0 0 40 125',
        crown: 'M32.0429 80.3319C25.3641 76.7153 12.3775 76.9648 8.17236 80.3319C2.73036 89.0619 -0.0157515 103.786 1.34475 110.396C2.70524 117.005 18.5365 125.61 21.2575 123.74C24.6402 121.414 38.3256 113.139 38.944 108.4C39.5624 103.661 34.8625 94.5572 34.6151 87.8227C34.4172 82.4352 32.9797 80.8392 32.0429 80.3319Z',
        roots: ['M9.01314 60.2618C8.22157 63.8534 8.10614 75.0609 8.14736 80.2156C17.5183 82.1917 23.0911 82.7369 33.6258 81.9617C33.5021 81.0887 32.5126 72.9825 31.6469 65.8739C30.7574 58.5708 31.0285 49.412 31.0285 46.5436C31.0285 43.6752 29.9153 33.3242 28.4311 29.5828C26.947 25.8415 27.5654 18.4834 26.8233 15.3656C26.0812 12.2478 20.8866 0.400168 17.9182 1.02373C14.9498 1.64728 17.1761 6.38633 16.1867 15.9891C15.3951 23.6714 12.8473 34.5296 11.9815 37.0654C10.7447 40.3079 10.0026 55.7721 9.01314 60.2618Z'],
        pulp: 'M18.6602 6.8971C23.0444 1.05663 24.7882 59.2963 24.4272 85.6725C24.406 87.219 23.1529 88.4437 21.619 88.4437C20.0082 88.4437 18.7249 87.0911 18.7917 85.4684C19.91 58.2791 19.4075 17.8055 18.6602 6.8971Z',
        gumLine: 'M 0,80 Q 20,88 40,80',
    },
    6: {
        viewBox: '0 0 64 128',
        crown: 'M52.5724 83.8192C38.6457 77.1735 21.2542 79.2177 11.7848 82.6763C9.77918 83.4088 8.23738 84.9707 7.35306 86.9098C5.1137 91.8204 1 101.871 1 109.443C1 114.19 5.86637 124.628 14.0266 126.184C20.4417 127.407 24.7345 122.822 31.8129 122.686C39.6605 122.535 44.3969 129.286 51.6033 126.184C58.803 123.084 64.5464 115.023 62.6258 109.443C60.8828 104.378 59.6213 92.6599 54.5345 85.8667C54.0084 85.1642 53.3957 84.2121 52.5724 83.8192Z',
        roots: ['M31.6876 1C25.1117 1 26.9905 28.8602 19.7883 35.2319C21.1244 45.8096 28.5513 59.3963 32.3139 60.4685C35.8211 61.468 44.0879 37.2308 45.7162 28.8602C41.0818 22.7385 37.1989 1 31.6876 1Z', 'M54.537 84.9802C54.537 83.5 52.5329 73.4613 54.537 64.8658C57.0421 54.1215 59.2967 40.8785 59.2967 31.3836C59.2967 22.4209 58.7387 10.8979 52.1445 5.22081C51.5917 4.74482 50.7688 5.08408 50.6455 5.80208C48.2891 19.5153 42.5034 48.131 33.2436 59.8685C30.5486 63.2845 16.6396 32.362 13.0019 10.5877C12.8671 9.78116 11.9139 9.48249 11.4381 10.1484C8.49442 14.2692 4.6997 23.8642 6.43885 39.1295C8.94396 61.1178 9.94601 59.2438 9.94601 68.364C9.94601 72.1459 8.4267 80.7768 7.6531 85.1369'],
        pulp: 'M37.1988 80.0831C37.6999 79.4584 46.3425 64.8414 48.7223 53.9721C51.1022 43.1029 55.3609 29.61 51.7285 16.6169C48.0961 3.62377 56.9892 42.1033 32.0634 74.836C28.5169 74.836 11.3588 40.418 11.9479 21.9296C11.9552 21.7004 11.6501 21.6422 11.583 21.8616C6.57857 38.2285 23.1191 71.6539 24.6733 74.8358C25.8632 77.2722 25.5083 79.75 25.4248 80.0831',
        gumLine: 'M 0,85 Q 32,95 64,85',
    },
    7: {
        viewBox: '0 0 66 124',
        crown: 'M57.655 80.8343C57.2557 80.5265 56.8413 80.2327 56.4129 79.9525C44.8747 72.4037 23.1621 74.6455 11.3233 76.9591C10.1919 77.2013 8.70714 77.8501 7.72107 78.7268C7.41105 79.0024 7.15033 79.3005 6.96539 79.6156C-7.5536 104.353 8.35603 122.835 13.4349 122.835C18.6519 122.835 21.1362 113.835 28.589 112.835C36.0418 111.834 46.4758 124.585 52.6865 122.835C57.5011 121.478 63.8657 109.459 64.8594 104.709C65.8531 99.9594 61.3815 83.8342 57.655 80.8343Z',
        roots: ['M23.1237 1.08372C17.9564 4.38375 17.6582 22.6256 18.1551 31.334C20.6394 39.2507 26.8998 54.484 32.0671 52.084C37.2344 49.684 39.1058 39.9172 39.3957 35.3338C34.1787 29.3338 30.2039 17.7089 30.2039 15.3339C30.2039 12.9588 28.2164 -0.166297 23.1237 1.08372Z', 'M10.79 62.9522C11.6845 68.8711 7 78.5 7.31157 78.7704C27.6459 73.526 38.316 73.7571 56.0109 80C58.3299 72.0579 59.4894 47.6533 54.5201 30.8499C48.5135 10.5393 40.5231 6.64772 37.9971 7.02392C34.767 9.65731 40.7302 23.326 39.7363 31.9786C39.2337 36.3543 34.1459 54.2997 31.1643 51.6663C22.468 45.0201 18.6789 33.6715 17.3744 30.8499C16.2812 28.4853 15.3867 18.31 15.014 15.6766C14.6413 13.0432 12.9641 8.27783 10.79 8.65412C8.61602 9.0304 5.86595 15.8311 4.45422 22.3228C3.33612 27.4642 4.45422 40.0042 5.94496 47.6535C7.37238 54.9779 9.79624 56.3761 10.79 62.9522Z'],
        pulp: 'M37.2839 74.5841C36.8698 73.8758 36.2902 72.1841 37.2839 71.0841C38.526 69.7091 46.9105 56.2089 48.5253 44.4588C51.1959 32.146 44.1778 14.3333 40.1408 10.7085C38.7124 12.271 46.9726 27.3962 45.1094 39.2088C43.9915 47.9589 39.8924 61.5835 31.3216 68.0838C25.7321 65.2713 18.3413 47.9586 14.4286 32.146C11.5574 23.0066 11.3488 14.4022 11.0748 14.3333C10.3295 14.1458 9.28612 26.6834 11.0748 35.5835C13.3106 46.7086 17.7823 56.8962 23.9309 68.0838C24.9246 72.7839 24.3657 73.7714 24.3657 75.0839',
        gumLine: 'M 0,78 Q 33,88 66,78',
    },
    8: {
        viewBox: '0 0 67 104',
        crown: 'M58.1322 63.4319C48.1437 53.9492 13.933 57.9421 8.18956 62.1842C5.31786 66.2599 -0.100915 76.682 1.19759 85.7652C2.33379 93.7129 8.96183 100.615 11.9353 100.987C17.9284 101.735 27.5423 95.3723 33.7851 96.3704C40.028 97.3686 44.0234 103.357 48.7679 102.983C53.5125 102.609 59.8802 99.3649 64.6247 92.8769C69.3693 82.1469 60.5045 70.7928 58.1322 63.4319Z',
        roots: ['M30.9758 1.09548C28.5411 1.78154 24.8995 7.08434 24.4209 9.70447C24.2544 20.4602 25.9192 41.1732 33.91 37.9792C41.9008 34.7851 41.4846 20.8761 40.2777 14.3209C39.5285 12.2622 34.1616 0.197776 30.9758 1.09548Z', 'M40.877 4.12291C58.0573 11.4094 58.1489 46.1284 56.9835 62.1402C38.3215 55.4866 27.3562 55.8281 7.04093 63C9.63795 60.5048 10.1207 54.0303 10.0375 52.1586C9.74852 45.6613 6.60782 42.3046 7.04093 31.8214C7.9399 10.0619 15.1566 3.54066 18.6526 3C24.1687 3 27.3814 27.6287 32.1621 35.9891C32.4602 36.5103 33.1208 36.5476 33.4394 36.0387C34.5725 34.2284 36.4464 30.1401 38.3799 24.5852C41.3765 15.9762 35.7579 2.25139 40.877 4.12291Z'],
        pulp: 'M42.0257 57.817C47.6443 47.9604 55.5102 24.5289 42.0257 9.65659C40.5274 7.78507 43.961 19.3885 43.3367 30.8048C42.5876 38.1661 40.5274 51.5788 34.6592 52.8265C31.08 51.9738 23.4096 46.8624 21.362 33.2378C19.558 21.2342 19.5274 12.3845 19.8019 8.34397C19.8479 7.66557 19.2514 7.21362 18.7915 7.71442C16.2381 10.4948 13.7475 20.5912 15.4313 33.2378C17.8659 44.0926 24.7955 56.7565 26.8556 57.817',
        gumLine: 'M 0,63 Q 33,73 67,63',
    },
};

const OCCLUSAL_SURFACES = {
    O: 'M 35,25 A 10,10 0 1,1 15,25 A 10,10 0 1,1 35,25 Z',
    B: 'M 32,18 L 41,9 A 22,22 0 0,0 9,9 L 18,18 A 10,10 0 0,1 32,18 Z',
    L: 'M 18,32 L 9,41 A 22,22 0 0,0 41,41 L 32,32 A 10,10 0 0,1 18,32 Z',
    M: 'M 18,18 L 9,9 A 22,22 0 0,0 9,41 L 18,32 A 10,10 0 0,1 18,18 Z',
    D: 'M 32,32 L 41,41 A 22,22 0 0,0 41,9 L 32,18 A 10,10 0 0,1 32,32 Z',
};

const OCCLUSAL_DIVIDERS = [
    'M 18,18 L 9,9',
    'M 32,18 L 41,9',
    'M 18,32 L 9,41',
    'M 32,32 L 41,41',
    'M 35,25 A 10,10 0 1,1 15,25 A 10,10 0 1,1 35,25',
];

const DEFAULT_STATUS_META = {
    healthy: { fill: '#ffffff', stroke: '#cbd5e1', accent: '#64748b', label: 'Healthy' },
};

const getPosition = (toothNumber) => toothNumber % 10;
const isUpperTooth = (toothNumber) => [1, 2].includes(Math.floor(toothNumber / 10));

const IMPLANT_BASE_WIDTH = 60;
const IMPLANT_BASE_HEIGHT = 72;
const IMPLANT_FIXTURE_PATH = 'M23,6 L18,62 Q30,72 42,62 L37,6 Z';
const IMPLANT_THREAD_PATHS = [
    'M22,14 L38,14',
    'M21,22 L39,22',
    'M20,30 L40,30',
    'M19.5,38 L40.5,38',
    'M19,46 L41,46',
    'M18.5,54 L41.5,54',
    'M19,62 L41,62',
];
const IMPLANT_COLLAR_PATH = 'M20,62 Q30,66 40,62 L42,67 Q30,73 18,67 Z';
const IMPLANT_HEAD_PATH = 'M23,3 Q30,-1 37,3 L37,8 Q30,11 23,8 Z';

function getImplantTransform(paths) {
    const [, , viewBoxWidth = 60, viewBoxHeight = 130] = paths.viewBox.split(/\s+/).map(Number);
    const gumMatch = paths.gumLine.match(/M\s*[\d.-]+,([\d.-]+)/);
    const gumY = gumMatch ? Number(gumMatch[1]) : viewBoxHeight * 0.7;
    const rootTop = Math.max(2, viewBoxHeight * 0.02);
    const rootBottom = Math.max(rootTop + 36, gumY + viewBoxHeight * 0.015);
    const rootHeight = rootBottom - rootTop;
    const scale = Math.min(rootHeight / IMPLANT_BASE_HEIGHT, (viewBoxWidth * 0.92) / IMPLANT_BASE_WIDTH);
    const translateX = (viewBoxWidth - (IMPLANT_BASE_WIDTH * scale)) / 2;
    const translateY = rootBottom - (IMPLANT_BASE_HEIGHT * scale);

    return `translate(${translateX.toFixed(2)}, ${translateY.toFixed(2)}) scale(${scale.toFixed(3)})`;
}

function getQuadrantClass(toothNumber) {
    const quadrant = Math.floor(toothNumber / 10);
    if (quadrant === 2) return styles.q2;
    if (quadrant === 3) return styles.q3;
    if (quadrant === 4) return styles.q4;
    return styles.q1;
}

function getStatusClass(stageKey) {
    if (stageKey === 'planned') return styles.stagePlanned;
    if (stageKey === 'completed') return styles.stageCompleted;
    return styles.stageExisting;
}

function LateralTooth({ toothNumber, toothData, statusMeta }) {
    const position = getPosition(toothNumber);
    const paths = LATERAL_PATHS[position] || LATERAL_PATHS[1];
    const scale = DISPLAY_CONFIG[position]?.scale || 0.7;
    const width = Math.round(55 * scale);
    const missing = toothData.status === 'missing' || toothData.status === 'extraction-site';
    const implant = toothData.status === 'implant';
    const affected = toothData.status !== 'healthy';
    const crownFill = affected && !missing ? statusMeta.fill : '#ffffff';
    const implantTransform = implant ? getImplantTransform(paths) : null;

    return (
        <svg
            width={width}
            height="118"
            viewBox={paths.viewBox}
            className={`${styles.lateralView} ${getQuadrantClass(toothNumber)} ${missing ? styles.transparentTooth : ''}`}
            aria-hidden="true"
        >
            <g className={styles.roots}>
                {!implant && paths.roots.map((rootPath, index) => (
                    <path key={`root-${index}`} d={rootPath} className={styles.toothRoot} />
                ))}
                {implant ? (
                    <g className={styles.implantRoot} transform={implantTransform}>
                        <path d={IMPLANT_FIXTURE_PATH} className={styles.implantFixture} />
                        {IMPLANT_THREAD_PATHS.map((threadPath, index) => (
                            <path key={`implant-thread-${index}`} d={threadPath} className={styles.implantThread} />
                        ))}
                        <path d={IMPLANT_COLLAR_PATH} className={styles.implantCollar} />
                        <path d={IMPLANT_HEAD_PATH} className={styles.implantHead} />
                    </g>
                ) : null}
            </g>
            <g className={`${styles.crown} ${getStatusClass(toothData.stage)}`}>
                <path d={paths.crown} className={styles.toothCrown} fill={crownFill} stroke={statusMeta.stroke} />
                {paths.pulp && !implant ? <path d={paths.pulp} className={styles.toothPulp} /> : null}
            </g>
            {toothData.status === 'root-canal' && paths.pulp ? (
                <path d={paths.pulp} className={styles.rootCanalFill} />
            ) : null}
            {toothData.status === 'fractured' ? (
                <path d="M12 92 L26 105 L20 116 L38 130" className={styles.fractureMark} />
            ) : null}
        </svg>
    );
}

function OcclusalTooth({ toothData, statusMeta }) {
    const surfaces = toothData.surfaces || [];
    const affected = toothData.status !== 'healthy';
    const wholeFill = affected && surfaces.length === 0 ? statusMeta.fill : '#ffffff';

    return (
        <svg width="44" height="44" viewBox="0 0 50 50" className={styles.occlusalView} aria-hidden="true">
            <path d="M 25,3 A 22,22 0 1,1 25,47 A 22,22 0 1,1 25,3 Z" className={styles.occlusalOutline} fill={wholeFill} stroke={statusMeta.stroke} />
            {Object.entries(OCCLUSAL_SURFACES).map(([surface, path]) => (
                <path
                    key={surface}
                    d={path}
                    className={`${styles.occlusalSurface} ${getStatusClass(toothData.stage)}`}
                    fill={surfaces.includes(surface) ? statusMeta.fill : 'transparent'}
                    stroke={surfaces.includes(surface) ? statusMeta.stroke : 'transparent'}
                />
            ))}
            {OCCLUSAL_DIVIDERS.map((path, index) => (
                <path key={index} d={path} className={styles.occlusalDivider} />
            ))}
            {toothData.status === 'missing' || toothData.status === 'extraction-site' ? (
                <g className={styles.occlusalMissing}>
                    <path d="M10 10 L40 40" />
                    <path d="M40 10 L10 40" />
                </g>
            ) : null}
            {toothData.status === 'implant' ? <circle cx="25" cy="25" r="8" className={styles.implantOcclusal} /> : null}
        </svg>
    );
}

function ToothCell({ toothNumber, toothData, statusMeta, readOnly, onSelect }) {
    const upper = isUpperTooth(toothNumber);
    const needsDistalSpacing = toothNumber === 18 || toothNumber === 48;
    const title = `Tooth ${toothNumber} - ${toothData.statusLabel || statusMeta.label}${toothData.surfaces?.length ? ` - ${toothData.surfaces.join(', ')}` : ''}`;

    return (
        <button
            type="button"
            className={`${styles.toothCell} ${upper ? styles.upperTooth : styles.lowerTooth} ${needsDistalSpacing ? styles.distalMolarStart : ''} ${toothData.status !== 'healthy' ? styles.toothAffected : ''}`.trim()}
            onClick={() => !readOnly && onSelect(toothNumber)}
            disabled={readOnly}
            title={title}
        >
            {upper ? (
                <>
                    <LateralTooth toothNumber={toothNumber} toothData={toothData} statusMeta={statusMeta} />
                    <OcclusalTooth toothData={toothData} statusMeta={statusMeta} />
                    <span className={styles.toothNumber}>{toothNumber}</span>
                </>
            ) : (
                <>
                    <span className={styles.toothNumber}>{toothNumber}</span>
                    <OcclusalTooth toothData={toothData} statusMeta={statusMeta} />
                    <LateralTooth toothNumber={toothNumber} toothData={toothData} statusMeta={statusMeta} />
                </>
            )}
        </button>
    );
}

function Quadrant({ teeth, getDisplayToothData, getStatusMeta, readOnly, onSelect }) {
    return (
        <div className={styles.quadrant}>
            {teeth.map((toothNumber) => {
                const toothData = getDisplayToothData(toothNumber);
                const statusMeta = getStatusMeta(toothData.status) || DEFAULT_STATUS_META.healthy;
                return (
                    <ToothCell
                        key={toothNumber}
                        toothNumber={toothNumber}
                        toothData={toothData}
                        statusMeta={statusMeta}
                        readOnly={readOnly}
                        onSelect={onSelect}
                    />
                );
            })}
        </div>
    );
}

function ArchRow({ leftLabel, rightLabel, leftTeeth, rightTeeth, getDisplayToothData, getStatusMeta, readOnly, onSelect }) {
    return (
        <div className={styles.archRow}>
            <span className={styles.archSideLabel}>{leftLabel}</span>
            <Quadrant teeth={leftTeeth} getDisplayToothData={getDisplayToothData} getStatusMeta={getStatusMeta} readOnly={readOnly} onSelect={onSelect} />
            <span className={styles.midline} />
            <Quadrant teeth={rightTeeth} getDisplayToothData={getDisplayToothData} getStatusMeta={getStatusMeta} readOnly={readOnly} onSelect={onSelect} />
            <span className={styles.archSideLabel}>{rightLabel}</span>
        </div>
    );
}

export function DentalPinToothPreview({ toothNumber, toothData, statusMeta, showNumber = false }) {
    const resolvedStatusMeta = statusMeta || DEFAULT_STATUS_META.healthy;

    return (
        <div className={`${styles.singleToothPreview} ${showNumber ? styles.withPreviewNumber : ''}`.trim()}>
            <OcclusalTooth toothData={toothData} statusMeta={resolvedStatusMeta} />
            <LateralTooth toothNumber={toothNumber} toothData={toothData} statusMeta={resolvedStatusMeta} />
            {showNumber ? <span className={styles.toothNumber}>{toothNumber}</span> : null}
        </div>
    );
}

export default function DentalPinStyleOdontogram({ getDisplayToothData, getStatusMeta, readOnly = false, onSelect }) {
    return (
        <div className={styles.chart} data-read-only={readOnly ? 'true' : 'false'}>
            <div className={styles.chartGrid}>
                <ArchRow
                    leftLabel="Upper right"
                    rightLabel="Upper left"
                    leftTeeth={UPPER_RIGHT}
                    rightTeeth={UPPER_LEFT}
                    getDisplayToothData={getDisplayToothData}
                    getStatusMeta={getStatusMeta}
                    readOnly={readOnly}
                    onSelect={onSelect}
                />
                <div className={styles.centerBand}>
                    <span />
                    <strong>FDI</strong>
                    <span />
                </div>
                <ArchRow
                    leftLabel="Lower right"
                    rightLabel="Lower left"
                    leftTeeth={LOWER_RIGHT}
                    rightTeeth={LOWER_LEFT}
                    getDisplayToothData={getDisplayToothData}
                    getStatusMeta={getStatusMeta}
                    readOnly={readOnly}
                    onSelect={onSelect}
                />
            </div>
            <p className={styles.creditNote}>Odontogram visual structure adapted with attribution to DentalPin.</p>
        </div>
    );
}
